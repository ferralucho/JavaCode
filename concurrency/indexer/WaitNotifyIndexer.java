 ...
 * 
 * Limitation:
 *    Solves task cooperation in a low-level fashion. 
...
public class WaitNotifyIndexer {
		...
		public void run() {
			try {
				synchronized (weblink) {
					...
					//Notify all wakes all the threads that are waiting that have executed the wait method on the object. 
					
				}		
                // *** lock released via wait() or exiting synchronized block. In order for the index thread to get to the runnable state it needs to acquire the lock ***		
	...
	private static class Indexer implements Runnable {
		...
		public void run() {	
			...
			synchronized (weblink) {//use the same lock
